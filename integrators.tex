...
Further, as demonstrated by Hochbruck et al.~\cite{Hochbruck:1997}, the matrix-exponential $\exp(\tau A)v$ obtained using Krylov subspace approximation converges faster than corresponding Krylov methods for the solution of linear equations $(I - \tau A)x = v$.
...
Finally, the adaptive time stepping procedures of all integrators used absolute and relative tolerances of \SI{1e-15} and \SI{1e-8}, respectively, throughout the work, and the exponential integrators used a rational approximant type of $\left(10,10\right)$ as suggested by Bisetti~\cite{Bisetti:2012jw}.
...
One aspect of the GPU platform particularly important to high performance algorithms are the unique memory access patterns; each streaming multiprocessor has only a small amount of fast cache memory (typically \SI{64}{\kilo\byte}) and \SI{32}{\bit} registers (typically \SIrange{32}{64}{\kilo\byte}) available to it.
All other memory is stored globally on the device, and the latencies are very high in comparison.
To get maximum performance out of the GPU, it is critical to properly utilize the fast memory cache.
When formulated on a per-block basis GPU-based integration algorithms have typically used this memory to store the current state vector, and other commonly used values (e.g. the chemical Jacobian).
However, on a per-thread basis the best use of this memory, is less clear; this section will explore one such approach for efficient use of this fast memory on a per-thread basis.
...
Algorithm~\ref{A:shared_mem_caching} describes an algorithm to use this available shared memory to store commonly used species concentrations during evaluation of the reaction rates. 
...
\begin{algorithmic}[0]
  \State {Initialize the cached species concentration set $C_{0} = \varnothing$; maximum set size is $C_{\max}$}
  \For {$r_i$ in reactions $R$}
    ...
    \State For each species $c_{i,j}$ in $C_i$ let $L_{i,j}$ be the number of reactions since $c_{i,j}$ 
    \Statex has directly participated in a reaction
    \State Priority sort the species $s_{i,j}$ in $S_{i}$ in descending order by priority $P_{i,j}$
    \Statex and store in $S_{i}^{\prime}$
    \For {$s_{i,j}$ in $S_{i}^{\prime}$}
      \If{$\|C_i\| < C_{\max}$}
	\State Add $s_{i,j}$ to $C_i$
      \ElsIf{$\max \left(L_{i}\right) \geq 2$ and $P_{i,j}$ > 1}
	  \State Let $k$ be the index of the maximum $L_{i,j}$ for all $c_{i,j}$ in $C_i$
	  \State Remove $s_{i,j}$ from $S_{i}^{\prime}$ and replace $c_{i,k}$ in $C_i$ with $s_{i,j}$
      \EndIf
    \EndFor
    \For {$s_{i,j}$ in $S_{i}^{\prime}$}
...
The CPU \texttt{Radau-IIa} integrator performed nearly equivalently compared with \texttt{CVODE} for the smaller global time-step size, and ran only slightly slower than \texttt{CVODE} for the larger time-step size.\todo{better to quantify these statements}
The GPU \texttt{Radau-IIa} integrator ran at best $8.41\times$ faster than \texttt{CVODE}, and on average performed $5.39\pm1.35\times$ faster for the smaller time step size (counting only runtimes for greater than 1024 ODEs, where the GPU became fully utilized).
Using the the larger time-step size negatively impacted the performance of the GPU \texttt{Radau-IIa} integrator---at best, it ran $2.69\times$ faster, and on average $1.78\pm0.81\times$ slower.
Both the CPU and GPU versions of the exponential integrators performed similarly to the CPU implicit integrators, and at the smaller time-step size the GPU versions generally outperformed their CPU counterparts.\todo{``by a factor of''?}
Although the \texttt{exprb43} and \texttt{exp4} algorithms each only require three exponential matrix function approximations, a single time step of \texttt{exprb43} is more expensive due to the extra chemical source term evaluations and matrix multiplications, and the higher-order phi function requirement.
As such, the CPU \texttt{exprb43} integrator generally outperformed the relatively simpler \texttt{exp4} integrator for the smaller time-step size.\todo{by?}
However, for the larger time-step size---with higher stiffness---the expected order reduction~\cite{Bisetti:2012jw} of the \texttt{exp4} integrator makes it less efficient than \texttt{exprb43}.\todo{this might require some elaboration, if possible}
...
The GPU \texttt{Radau-IIa} integrator often outperformed the CPU version and \texttt{CVODE} for the smaller time-step size; at best, it ran $1.46\times$ faster, and on average $1.13\pm0.63\times$ slower, than \texttt{CVODE}.
The larger time-step size again negatively impacted the performance of the GPU-based \texttt{Radau-IIa} integrator, dropping the performance to $8.63\times$ slower in the best case, and $14.18\pm6.64\times$ slower on average.\todo{compared to CPU version, or CVODE?}

Two primary factors affect the performance of the GPU integration algorithms: chemical stiffness and thread divergence.
The CPU-based \texttt{Radau-IIa} integrator demonstrated almost identical performance to \texttt{CVODE} except for the GRI-Mech.~3.0 model with the larger time step---the stiffest case.
...
Furthermore, the relative performance difference of the GPU \texttt{Radau-IIa} solver between the \ce{H2}\slash\ce{CO} and GRI models at the smaller time-step size is likely partially due to the increased thread divergence caused by different chemical timescales between the models.
Furthermore, increased thread divergence likely caused the relative performance difference of the GPU \texttt{Radau-IIa} solver between the \ce{H2}\slash\ce{CO} and GRI models at the smaller time-step size, due to differing chemical timescales.
...
Figure~\ref{F:GRI_perf} also shows that the CPU exponential integrators perform similarly to the CPU implicit integrators for the smaller time-step size.
As the exponential integrators must only factorize a much smaller Hessenberg matrix rather than the entire Jacobian---as in the implicit integrators---significant savings in factorization can potentially be achieved with larger mechanism sizes.\todo{can you put a number to how much smaller the Hessenberg is compared to the Jacobian?}
...
We hypothesize that the exponential integrators may be more efficient than the implicit integrators for moderately stiff systems (e.g., with small time-step sizes, or low tolerances) with larger chemical mechanisms.\todo{should mention RKC?}
...
Thus, we recommend using this or a similar caching scheme in order to exploit shared memory in GPU integrators.
\todo{or something like this to conclude the section?}
...
However, for larger models, e.g., \SIrange{250}{500} species, this issue negatively impact performance by forcing kernel launches small enough to underutilize the GPU resources.
...
Finally, an issue was identified with storage of the Jacobian and similar matrices in per-thread local memory, and the balance between independent ODEs per kernel launch and maximum mechanism size was discussed.\todo{still true?}

When compared with the results of a similar integration technique by Sewerin et al.~\cite{Sewerin20151375}, it is clear that the use of an analytical chemical Jacobian matrix greatly improves implicit integration speeds on the GPU.
