...
The larger time-step size again negatively impacted the performance of the GPU-based \texttt{Radau-IIa} integrator, dropping the performance to $\SI{8.63}{\times}$ slower than \texttt{CVODE} in the best case, and $\SI{17.98\pm13.04}{\times}$ slower on average.
...
Figure~\ref{F:GRI_perf} also shows that the CPU exponential integrators are only $\SIrange{2}{3}{\times}$ slower than to \texttt{CVODE} for the smaller time-step size.
As the exponential integrators must only factorize a much smaller Hessenberg matrix, typically a $\sim 5\times 5$ matrix or so for small integrator time-steps in stiff cases, rather than the entire Jacobian---as in the implicit integrators---significant savings in factorization can potentially be achieved with larger mechanism sizes.
For larger integrator time-steps without large chemical stiffness (e.g. near equilibrium) the size of the Hessenberg matrix to be factorized grows to a more typical $\sim 20\times 20$.
...
It is also noted that previous work~\cite{Niemeyer:2014aa} has shown that stabilized explicit integrators can provide significant speedups for moderately stiff systems.
