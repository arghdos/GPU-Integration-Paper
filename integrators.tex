...
\begin{figure}
  ...
  CPU versions were run in parallel on 6-cores.}
  \label{F:H2_perf}
\end{figure}
...
Although the \texttt{exprb43} and \texttt{exp4} integrators each only require three exponential matrix function approximations, \texttt{exprb43} is more expensive for a single step larger due to the extra right hand side evaluations / matrix multiplications, and the higher order phi function requirement.
...
\begin{figure}
  ...
  \begin{subfigure}{0.48\textwidth}
      ...
      \caption{$\delta t = \SI{1e-4}{\sec}$}
  \end{subfigure}
  \caption{Average runtimes of the integrators for the \ce{GRI} mechanism at two different global timesteps. 
  CPU versions were run in parallel on 6-cores.
...
Niemeyer et al.~\cite{Niemeyer:2014aa} investigated thread divergence of stabilized explicit integrators for both the \ce{H2} and GRI mechanisms used in this study, 
