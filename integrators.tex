...
In order to present unbiased speedup\slash slowdown numbers, in this section the average speedup will be computed for cases where the computational resources become fully utilized, that is when the runtime begins to increase linearly with the number of ODEs; \num{1024} ODEs was used for the \ce{H2} mechanism while \num{512} was used for GRI.
...
However the nominal fourth-order convergence of the \texttt{exp4} algorithm is a classical non-stiff order, and order reduction is expected for stiff problems~\cite{ANU:7701740,Bisetti:2012jw}.
Correspondingly for the stiffer, larger time-step case the CPU \texttt{exprb43} outperforms \texttt{exp4} by \SI{49\pm1}{\percent} on average.
