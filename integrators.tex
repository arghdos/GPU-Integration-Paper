...
An algorithm to utilize this available shared memory is outlined by example for the reaction rate subroutine in~\ref{A:shared_mem_caching}

\begin{algorithm}
\caption{Shared memory caching during evaluation of the reaction rates.}
\begin{algorithmic}[0]
  \For {$rxn$ in reactions}
    \State Let $S_{rxn}$ be the set of participating species in $rxn$
    \For {$S_{rxn,i}$ in $S_{rxn}$}
      \State Let $P_{rxn, i}$ be the number of consecutive reactions $S_{rxn,i}$ participates in.
    \EndFor
    \State Let $S_{rxn}^{\prime}$ be the the set $S_{rxn}$ sorted in ascending descending order by $P_{rxn, i}$
  \EndFor
\end{algorithmic}
