...
The integration step-size was then varied from \SIrange{e-6}{e-11}{\second} to measure the convergence rates of the various solvers; adaptive time-stepping was turned off in this section.

Figure~\ref{F:convergence} shows the convergence plot for the CPU solvers.
The error of the \texttt{Radau-IIA} integrator is only consequential for the largest integration time-step size (\SI{e-6}{\second}), reducing the time-step size to \SI{e-7}{\second} results in a reduction in error roughly four orders in magnitude, nearing the nominal fourth order of the method.
Further decrease of the time-step does not result in further decreases in error, due to numerical precision effects.
...
As the time-step size is decreased, the convergence of the Arnoldi algorithm is affected by the internal integration time-step size (the matrix exponentials and error estimates are scaled by the internal step size).
...
The \texttt{exprb43} solver exhibits roughly third order convergence with the ``exact'' Krylov subspace; it is unclear whether an order-reduction has occurred or this is due to numerical precision effects.
...
We studied the performance of the integrators by testing each on the PaSR conditions described in Section~\ref{S:pasr_conditions} for two different global integration time-step sizes: $\delta t = \SI{e-6}{\s}$ and $\delta t = \SI{e-4}{\s}$, representative of those used in large eddy~\cite{Wang20111319,Bulat20133155} and Reynolds-averaged Navier--Stokes simulations, respectively.\todo{may be good to add citations for these numbers, so that they don't look like we pulled them out of midair}
...
To this end, Figs.~\ref{F:H2_perf} and \ref{F:CH4_perf} show vertical lines where the runtime per ODE is first within \SI{10}{\percent} the cost per ODE when simulating the entire database (based on \texttt{CVODE}\slash\texttt{Radau-IIA} for the CPU\slash GPU accordingly).
...
Indeed, Section~\ref{S:results} notes that we suspect thread divergence from differing Krylov subspace sizes as the reason the exponential solvers are less efficient for small numbers of ODEs for the \ce{H2}\slash\ce{CO} model with the small time-step size.
...
Thread divergence may be an even more important concern for larger chemical kinetic models that exhibit even more chemical stiffness than the GRI-Mech 3.0 model.
...
For the larger time-step the \texttt{exprb43} divergence measure increases significantly for both models.
...
The explicit solvers are less efficient at dealing with stiffness, and end up using a greater range of time-step sizes between stiff and non-stiff conditions resulting in an increase in thread divergence levels due to differing internal time-steps.

The expected order reduction of the \texttt{exp4} method is also apparent in Figs~\ref{F:exp4_div_h2} and \ref{F:exp4_div_ch4}, in which exhibits significantly thread divergence than either of the other solvers for most cases.
...
Dijkmans et al.~\cite{Dijkmans:2014bb} used a GPU-based analytical Jacobian code to accelerate various CPU-based chemical kinetic integration schemes, and our own previous works~\cite{Niemeyer:2016aa,Niemeyer:2015ws} have detailed the performance of \texttt{pyJac}, however to our knowledge no work using an analytical Jacobian for GPU-based chemical kinetic integration has been published.
In this section, we explore the relative performance benefits of the analytical Jacobian compared to a first-order finite-difference-based Jacobian on both the CPU and GPU.
...
For the \ce{H2}\slash\ce{CO} model (Figs.~\ref{F:AJ_h2_small} and~\ref{F:AJ_h2_large}), using the analytical Jacobian offers minimal performance benefit for the CPU-based integrators, reaching a maximum speedup of \num{1.58} and \num{1.49}$\times$ for the small and large time-steps respectively.
...
However, for large numbers of conditions, the analytical-Jacobian-based solver indeed performs faster than the finite-difference counterpart; for the full PaSR database, the analytical Jacobian results in speedups of \num{1.58} and \num{1.36}$\times$ for the small and large time-steps respectively.
In contrast, the analytical-Jacobian-based GPU solver performs significantly faster than the finite-difference GPU solver in all cases for the \ce{H2}\slash\ce{CO} model, reaching a maximum speed up of \num{11.72}$\times$.
...
Correspondingly, the maximum speedup of the GPU solver increases to \num{241.08}$\times$ for the larger time-step.
Figure~\ref{F:AJ_ch4_small} shows that the speedup for the CPU solver reaches \num{2.52}$\times$ for the larger GRI-Mech 3.0 model at the smaller time-step size.
Similar to the \ce{H2}\slash\ce{CO} case with the smaller time-step, the analytical-Jacobian-based GPU \texttt{Radau-IIA} solver reaches a maximum speedup of \num{8.09}$\times$ faster for the GRI-Mech 3.0 Model.
...
The exponential solvers performed significantly worse than the implicit integrators on the CPU and GPU in all relevant cases.
