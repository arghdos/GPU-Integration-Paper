...
This effort represents the state-of-the-art of GPU-accelerated stiff chemical kinetic evaluation, we therefore additionally investigate the potential impacts of the current results on large-scale reactive-flow simulations in this section.
This work is a starting point on a path to reduce the cost of reactive-flow simulations with realistic chemistry; to this end we identify the most promising future directions for GPU\slash SIMD accelerated chemical kinetic integration in Sec.~\ref{S:conclusions}.
...
As will be discussed in Sec.~\ref{S:divergence}, this drop in performance is due to increased levels of thread divergence and higher memory traffic requirements.
Although this work represents the current state-of-the-art for implicit integration of stiff chemical kinetic ODEs on the GPU, it is clear that more effort is required to improve performance for larger, stiffer mechanisms.
