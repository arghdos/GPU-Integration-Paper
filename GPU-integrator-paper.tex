...
Use of a newer GPU model may significantly improve solver performance for larger models due to the scarcity of on-chip memory in a per-thread approach.
Finally, a per-block approach may be required for efficient integration of the largest models on the GPU, due to the much larger amounts of cache memory allowed for each ODE solution.

On the other hand, the performance penalty due to thread divergence depends both on the cost of the divergent branches as well as the proportion of the warp that executes each branch.
