...
\begin{itemize}
 ...
 \item \hlb{Intel's Xeon Phi co-processor has tens of (hyperthreaded) cores with wide-vector units designed for SIMD execution, each core capable of running multiple independent threads}
\end{itemize}
...
\hlg{All threads in a warp are executed while any thread in the warp is unfinished; if a divergent path is long and complicated, or if only a handful of threads in a warp require its execution, significant computational wastage may occur as the other threads will be idle for long periods.}
...
\bottomrule
\end{tabular}
\caption{The solvers used in this study, and platforms implemented on.}
\label{T:solvers}
\end{table}
...
Further characterization of the PaSR conditions used in this work can be found in~\ref{S:pasr_charac} and our previous work~\cite{Niemeyer:2016aa}.
...
Increasing the number of integration steps---by further reducing the internal time-step size---past this point does not reduce error further, due to numerical precision effects; \hlb{the {\texttt{Radau-IIA}} solver is nominally fifth-order, as such it is unclear whether order-reduction has occurred due to the stiffness of the problem or whether this is merely an ``apparent'' order-reduction due to numerical precision effects.}
\hlb{A more accurate assessment of convergence order might be achieved through use of a stiff sample problem with an analytical solution---e.g. HIRES or ROBER---however, we prefer use of a chemical kinetics problem for direct validation of the work at hand.}
...
The \texttt{exprb43} solver reaches third-order convergence with the ``exact'' Krylov subspace---\hlb{similar to the discussion on the {\texttt{Radau-IIA}} convergence order, it is difficult to determine whether this is due to numerical precision effects or actual order-reduction has occurred due to problem stiffness}.
...
As an aside, it is further noted that Bisetti~\cite{Bisetti:2012jw} demonstrated a method to exploit the underlying sparsity of a dense mass-fraction-based constant-pressure Jacobian matrix (used in this study) to accelerate Jacobian-vector multiplications, however a reformulation is still more attractive as it enables sparse-LU factorization.
...
\section{Partially Stirred Reactor Data Characterization}
\label{S:pasr_charac}
\begin{figure}[htbp]
  \centering
  \begin{subfigure}{0.49\textwidth}
...
\end{figure}
\hlb{Fig.~{\ref{F:pasr_characterization}} shows an example characterization of the PaSR conditions sampled for validation and performance testing in this work; a wide range of temperatures and fuel-mass fractions are observed for both models.}
\hlb{It is noted that significantly more diversity in fuel mass-fraction is seen for the GRI-Mech 3.0 near the adiabatic flame temperature.}
\hlb{Further, the hydrogen model is more sensitive to the mixing\slash pairing time-scales, showing many large temperature changes quickly recovering to a semi-stable temperature.}
