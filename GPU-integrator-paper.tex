...
In contrast, the analytical-Jacobian-based GPU solver performs significantly faster than the finite-difference GPU solver in all cases for the \ce{H2}\slash\ce{CO} model, reaching a maximum speed up of \SI{11.72}{$\times$}.
...
Figure~\ref{F:AJ_ch4_small} shows that the speedup for the CPU solver reaches \SI{2.52}{$\times$} for the larger GRI-Mech 3.0 model at the smaller time-step size.
Similar to the \ce{H2}\slash\ce{CO} case with the smaller time-step, the analytical-Jacobian-based GPU \texttt{Radau-IIA} solver reaches a maximum speedup of \SI{8.09}{$\times$} faster for the GRI-Mech 3.0 Model.
