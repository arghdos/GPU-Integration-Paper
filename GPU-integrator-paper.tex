...
For the \ce{H2}\slash\ce{CO} model (Figs.~\ref{F:AJ_h2_small} and~\ref{F:AJ_h2_large}), the performance benefit from use of the analytical Jacobian is fairly minimal for the CPU based integrators reaching speedup of only $\sim\mathcal{O}(1\times)$ for both time-step sizes.
...
The performance of the analytical Jacobian based GPU \texttt{Radau-IIA} solver is again significantly better than its finite-difference counterpart in all cases.
...
As the finite-difference Jacobian evaluation is relatively costly, we expect a large drop in efficiency when thread divergence is encountered; if only a few threads require Jacobian updates, the SM will be underutilized for long periods.
