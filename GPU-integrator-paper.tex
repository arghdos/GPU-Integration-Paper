...
The error of the \texttt{Radau-IIA} integrator is only consequential for the largest integration time-step size (\SI{e-6}{\second}), reducing the time-step size to \SI{e-7}{\second} results in a reduction in error roughly four orders in magnitude.
Further decrease of the time-step does not result in further decreases in error, due to numerical precision effects; this is likely the cause of the apparent order reduction of the \texttt{Radau-IIA} solver, which is nominally fifth order.
