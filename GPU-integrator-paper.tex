...
Using an analytical Jacobian on the GPU was found to be critical for efficient integration, resulting in maximum speedups of \SIrange{8}{241}{$\times$}, far surpassing the \SIrange{1.5}{2.5}{$\times$} speedup achieved with the analytical Jacobian CPU for identical problems.
...
In reacting flow simulations, the chemical integration time-step is typically determined by the flow time-scale and stability requirements determined by the Courant--Friedrichs--Lewy number; typical time-step values range from \SIrange{e-6}{e-1}{\s}~\cite{Yang:2013ip}.
