...
\journal{\todo[inline]{decide on journal}}
...
We estimate that the implicit Runge--Kutta method on a single GPU is roughly equivalent to \texttt{CVODE} running on 10 or more CPU cores for all but the stiffest case studied, where performance was greatly decreased by thread divergence.
...
Unlike the performance for the \ce{H2}\slash\ce{CO} model, when using the GRI-Mech 3.0 model the \texttt{exprb43} solver outperforms \texttt{exp4} for all but the CPU solvers with the smaller time-step (for which the performance is similar).
...
Further, the GRI-Mech 3.0 model experiences higher levels of thread divergence for the larger time-step, and subsequently a larger drop in performance.
...
This is expected as the explicit solvers are less efficient at dealing with stiffness, and further helps explain the large performance gap between the \texttt{Radau-IIA} solver and the exponential methods for the \ce{H2}\slash\ce{CO} model for the two time-steps (Figs.~\ref{F:h2_gpu_perf_small} and \ref{F:h2_gpu_perf_large}).
...
Correspondingly, maximum speedups of $\sim\mathcal{O}(10\times$) are observed for both models with the smaller time-step, while much larger speedups, $\sim\mathcal{O}(100\times)$, are observed for the \ce{H2}\slash\ce{CO} model with the larger time-step.
...
\fi
\todo{figure out pre-print style}
