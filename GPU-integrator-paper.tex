...
We estimated that the implicit Runge--Kutta method running on a single GPU is equivalent to \texttt{CVODE} running on \numrange{11}{38} CPU cores for integration of a single global integration time step of \SI{e-6}{\second} with hydrogen\slash air and methane\slash air models.
...
\midrule
\SI{e-6}{\second} & 38.30 & 11.69 \\
\SI{e-4}{\second} & 14.66 & 2.96 \\
\bottomrule
\end{tabular}
...
\begin{itemize}
 ...
 \item For the \ce{H2}\slash\ce{CO} model with a time-step size relevant to Reynolds-averaged Navier--Stokes simulations, the GPU-based Runga--Kutta solver performed equivalent to \texttt{CVODE} running on \num{14} cores; for the GRI-Mech 3.0 model, the Runge--Kutta solver performance was only equal to \texttt{CVODE} on $\sim$\num{3} cores.
