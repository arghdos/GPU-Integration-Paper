...
\end{equation}
where the $y_i(t)$ is the solution obtained from the various solvers, and $\hat{y}_i(t)$ is the ``true'' solution obtained via \texttt{CVODE} using a time-step of $\delta t = \num{e-10}$ and absolute\slash relative tolerances of \num{e-20} and \num{e-15} respectively.
The maximum error over all conditions:
\begin{equation}
 \lvert\textbf{E}\rvert = \max_{i=1\dots1000}\{E_i(t)\}
\end{equation}
...
Adaptive time-stepping was used in all cases, and Krylov subspace approximation was enabled for the the exponential integrators thus evaluating the accuracy of the solvers, in exact form as used for performance testing.
The global integrator time-step was varied from \SI{e-6}{\second} to \SI{e-11}{\second} to generate the convergence plot, Fig.\ref{F:convergence}.
