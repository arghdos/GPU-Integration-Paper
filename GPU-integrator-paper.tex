...
In this work, the NVIDIA CUDA framework~\cite{Buck:2008aa,NVIDIA:2015aa} was used, hence the following discussion will use CUDA terminology; however the concepts within are widely applicable to SIMD processing.
...
The threads in a warp are executed in parallel on CUDA cores, which multiple warps are typically executed concurrently.
...
Further, GPUs have relatively small memory caches and few registers available per SM, as compared to a typical CPU.
...
The largest potential benefit of a per-thread approach is that a much larger number of ODEs can theoretically be solved concurrently; typically the number of blocks that can be executed concurrently on each SM is $\sim\num{8}$, whereas typical CUDA launch configurations in this work consist of 64--128 threads per block.
...
Finally we note that past work has shown that in the best-case scenario, elimination of the majority of thread divergence by choice of identical initial conditions, the performance of a per-thread implicit integration algorithm  surpassed that of the per-block implementation of the same algorithm~\cite{Stone:2013aa}.
...
One goal of GPU-based chemical kinetic integration is to enable use of larger detailed mechanisms in high-fidelity reactive-flow simulations.
However, storage of the Jacobian on the GPU places severe restrictions on either the maximum allowable model size or the total number of independent chemical-kinetic ODEs that can be solved concurrently.
...
Similarly, the exponential integrators require storage of matrices for the Jacobian, Hessenberg and vector subspace resulting from the Arnoldi process, and corresponding Hessenberg exponential.
...
Another option involves preallocating the Jacobian and associated matrices in global memory, thus splitting the total global memory between all ODEs to be solved in a kernel launch.
...
These limits would severely constrain the allowable model sizes; for example, the \texttt{Radau-IIA} algorithm could handle models with approximately 15, 50, and 158 species for \num{e6}, \num{e5}, and \num{e4} concurrent ODEs, respectively.
...
Sewerin and Rigopoulos~\cite{Sewerin20151375} discussed one approach to alleviate this issue: reducing the number of ODEs solved per kernel launch.
...
Otherwise, model reduction (a priori or dynamic) may be employed to bypass this complication.
