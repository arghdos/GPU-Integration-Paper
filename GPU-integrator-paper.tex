...
For validation and performance assessments of \texttt{pyJac}, we direct readers to our previous work~\cite{Niemeyer:2015ws,Niemeyer:2016aa}.
...
In conjunction, the convergence of the Arnoldi algorithm is affected by the internal integration step-size (the matrix exponentials and error estimates are scaled by the internal step-size).
...
Compared to the \ce{H2}\slash\ce{CO} model (Fig.~\ref{F:h2_cpu_perf_small}), the CPU  implicit integrators perform significantly better than the exponential algorithms for the GRI-Mech 3.0 model with the small time-step (Fig.~\ref{F:ch4_cpu_perf_small}); a result of the higher stiffness present in the model.
...
Investigation of thread divergence for the operations within a internal integration step is an important consideration and will be explored in our future work.
...
It is well established that use of an analytical Jacobian can significantly accelerate chemical kinetic integration on the CPU~(e.g., \cite{Lu:2009gh,stone2014comparison,Schwer2002270}), to our knowledge no work utilizing an analytical Jacobian for GPU-based chemical kinetic integration has been published.
...
As the exponential methods are formulated for use with an exact Jacobian (rather than approximate finite-difference based Jacobians), their performance was not considered during this section.
...
For the \ce{H2}\slash\ce{CO} model (Figs.~\ref{F:AJ_h2_small} and~\ref{F:AJ_h2_large}), the performance benefit from use of the analytical Jacobian is fairly minimal for the CPU based integrators reaching speedup of only $\mathcal{O}(\num{1}\times)$ for both time-step sizes.
This is due to both the relatively small size of the model and modest resulting chemical source term evaluation cost, as well as the lower chemical stiffness which requires only a few Jacobian evaluations for the \texttt{Radau-IIA} solver.
...
In contrast, the analytical Jacobian based GPU solver is significantly faster than the finite-difference GPU solver in all cases for the \ce{H2}\slash\ce{CO} model.
Further, from Sec.~\ref{S:divergence}, significantly higher levels of thread divergence are expected for the larger time-step size.
Correspondingly, a maximum speedup of $\mathcal{O}(\num{10}\times)$ is observed on the GPU for the smaller time-step, while a much larger speedup, $\mathcal{O}(\num{100}\times)$, is observed with the larger time-step.
In Fig.~\ref{F:AJ_ch4_small} we see that the speedup for the CPU solver reaches \SIrange{2}{3}{$\times$} for the larger, more stiff GRI-Mech 3.0 model using the small time-step.
Similar to the \ce{H2}\slash\ce{CO} case with the smaller time-step, the analytical Jacobian based GPU \texttt{Radau-IIA} solver is $\mathcal{O}(10\times)$ faster for the GRI-Mech 3.0 Model.
