...
The reason for this is the modest stiffness of the \ce{H2}\slash\ce{CO} model; even with many near-equilibrium states removed from the beginning of the PaSR database, the model is not particularly stiff for this small time-step.
...
As Fig.~\ref{F:h2_cpu_perf_small} does not show increased solution cost at the corresponding locations (e.g. as for the maximum number of ODEs), it can be surmised that these increased costs are due to thread-divergence, rather than increased chemical stiffness.
This pattern of thread-divergence, which will be explored further in Sect.~\ref{S:divergence}, is exhibited to some extent for all GPU integrators in this work.
Interestingly, the \texttt{Radau-IIA} solver is not as negatively affected by thread-divergence in this range as the exponential integrators.
...
This will be investigated in Sect.~\ref{S:divergence}.
...
Unlike the performance for the \ce{H2}\slash\ce{CO} model, when using the GRI-Mech 3.0 model the \texttt{exprb43} solver outperforms \texttt{exp4} for all but the CPU solvers with the smaller time-step.
...
In this light we ask, in a given a large-scale reacting flow simulation, what is the effect on the overall runtime of adding more CPU cores v.s. adding GPU accelerators?
...
To this end, Figs.~\ref{F:H2_perf} and \ref{F:CH4_perf} have been marked with a vertical line in all cases where it is estimated that the runtime per ODE has reached the limit performance.
These choices are somewhat arbitrary (it could be argued that they should be moved up or down a point), however this section does not seek to provide an exacting performance comparison, rather general trends.
...
\midrule
\SI{e-6}{\second} & 36.6 & 10.4 \\
\SI{e-4}{\second} & 13.0 & 2.6 \\
\bottomrule
\end{tabular}
