...
\hlg{Several recent studies~\cite{Huang20091814,Bottone2012,Moiz2016123} demonstrate that even modestly sized chemical kinetic models can incur severe computation cost for realistic reactive-flow simulations; e.g., in~\cite{Moiz2016123} a single LES realization of a diesel spray--using a 54-species n-dodecane model---for {\SI{2}{\milli\second}} after start of injection took {\num{48000}} CPU core hours and up to {\num{20}} days of wall clock time.}
...
In this work, our efforts to accelerate simulations with chemical kinetics focus on improving the integration strategy itself, via development of new algorithms for high-performance hardware accelerators such as graphics processing units (GPUs) and similar single-instruction multiple-data\slash thread (SIMD\slash SIMT) devices, \hlg{available on increasing numbers of US and international supercomputing clusters~\cite{nvidia,xsede,orlcf}}.
...
A per-thread approach may encounter more cache-misses, since the memory available per SM must now be split between many more sets of IVPs.
